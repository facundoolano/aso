...
// TODO factor out score calculations
...
function isCompetitor (keyword, app) {
  return getKeywords(app.description).then((kws) => R.contains(keyword, kws.slice(0, 10)));
}
...
function getCompetitors (keyword, apps) {
  return Promise.all(apps.map((app) => isCompetitor(keyword, app)))
    ...
    .then(R.length)
    .then((count) => ({count, score: 10 * count / apps.length}));
}
...
function getInstalls (keyword, apps) {
  ...
  const avgForScore = Math.min(avg, ceiling);
  const score = 1 + 9 * (avgForScore) / ceiling;
  return {avg, score};
}
...
function getDaysSince (dateString) {
  return Math.floor((Date.now() - Date.parse(dateString)) / 86400000);
}
...
function getAge (keyword, apps) {
  ...
  const avg = R.sum(updated);
  const ceiling = 900;
  const avgForScore = Math.min(avg, ceiling);
  const score = 1 + (ceiling - avgForScore) * 9 / ceiling;
...
function getScore (stats) {
  const sum = TITLE_W * stats.titleMatches.score +
              COMPETITOR_W * stats.competitors.score +
              DOWNLOAD_W * stats.downloads.score +
              RATING_W * stats.rating.score +
              AGE_W * stats.age.score;
  const ceiling = 10 * (TITLE_W + COMPETITOR_W + DOWNLOAD_W + RATING_W + AGE_W);
  return 1 + 9 * sum / ceiling;
}
      ...
      return getCompetitors(keyword, apps) // gimme freakin destructuring
        .then(function (competitors) {
          ...
          return {
            ...
            downloads: getInstalls(keyword, topApps),
