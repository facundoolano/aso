...
const throttled = (method) => (opts) => method(R.assoc('throttle', 20, opts));
...
function getSuggestLength (keyword, length) {
  ...
  return throttled(gplay.suggest)({term: prefix})
...
const store = {
  ...
  list: throttled(gplay.list),
  search: throttled(gplay.search),
  app: throttled(gplay.app),
  similar: throttled(gplay.similar),
  suggest: (term) => throttled(gplay.suggest)({term}),
