  ...
  */
  ...
  function getDaysSince (dateString) {
    ...
    if( typeof dateString == "number" ) {
        // epochdatetime when country: us
        app_date = new Date(0);
        app_date.setUTCSeconds(dateString/1000);
    } else {
        // for some reason on country: es datetime comes with datetime parse
        app_date = Date.parse(dateString);
    }
    return Math.floor((Date.now() - app_date) / 86400000);
  }
